...
const DEFAULT_FREQ_FIELD_NAME = "__topic_frequency"; // virtual field used to calculate windows on top of message rate per topic
const DEFAULT_FREQ_WINDOWS = ["15m", "12h", "1d"]; // windows for the virtual frequency field
...
class DSLHandler {
    ...
    constructor(config = {}, sarkac = null){
        this.config = config;
        // ensure clone
        this.dsl = this.config.dsl ? JSON.parse(JSON.stringify(this.config.dsl)) : {};
        this.sarkac = sarkac;
        ...
        this.freqAnomalyDetectionActive = 
            typeof this.config.freqAnomalyDetectionActive === "boolean" ? 
            this.config.freqAnomalyDetectionActive : true;

        this.freqFieldName = this.config.freqFieldName || DEFAULT_FREQ_FIELD_NAME;
        this.freqFieldWindows = this.config.freqWindows || DEFAULT_FIELD_WINDOWS;
    }
    ...
    async handleMessage(message, stream){
        ...
        // create and store virtual field
        
        const fieldHandlePromises = fields.map((field) => this.handleField(field, message, stream));

        if(this.freqAnomalyDetectionActive){
            // push a fake field for the message frequency of the current topic
            fieldHandlePromises.push(this.handleField(
                {
                    path: this.freqFieldName,
                    windows: this.freqFieldWindows,
                },
                {
                    [this.freqFieldName]: 1,
