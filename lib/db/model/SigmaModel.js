...
class SigmaModel {
    ...
    getCountOfAvailableEventsInWindow(topic, field, windowSeconds){
        const key = this._getKey(topic, field);
        return this.model.aggregate([
            {
                $match: {
    ...
    getTotalCountOfEvents(windowSeconds = 628000000){
        debug("Counting events for a window of", windowSeconds);
        return this.model.aggregate([
            {
                $match: {
                    produced: { $gte: Date.now() - windowSeconds * 1000 }
    ...
    clearCollection(){
        debug("Deleting the total collection");
        return this.model.remove({});
    }
}
